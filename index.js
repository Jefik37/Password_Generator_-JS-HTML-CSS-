...
HTML_CODE = function(variable) {
    return `<div style = " margin: auto;width: 80%; background-color: white; margin-top: 20px; padding: 20px;
        ...
        <p style = 'margin: 0px; padding: 0px; font-family: "cascadia code semibold", monospace; font-size: 1.4em;'>${variable}</p></div>`
}
...
SUBMIT_OPT1.onclick = function() {
    ...
    const ERR_MSG2 = "The number of elements in the password must be between 8 and 50."
    ...
    if (!PERMISSION_SYMBOLS && !PERMISSION_UPPER && !PERMISSION_NUMBERS && !PERMISSION_LOWER) {
        ...
        setTimeout(function() {
            let password = genPassword(PASSWORD_SIZE, MUST_BE, PERMISSION_SYMBOLS, PERMISSION_UPPER, PERMISSION_LOWER, PERMISSION_NUMBERS);
            let RESPONSE1_HTML = HTML_CODE("&#x1F513; Your password >> " + password);
...
SUBMIT_OPT2.onclick = function() {
    ...
    const ERR_MSG2 = "The number of elements inside the text must be between 8 and 50.";
    ...
    if (size < 8 || size > 50) {
        ...
        setTimeout(function() {
            let MESSED_PASSWORD = shuffleString(INPUT);
            let RESPONSE2_HTML = HTML_CODE("&#x1F513; Your password >> " + MESSED_PASSWORD);
